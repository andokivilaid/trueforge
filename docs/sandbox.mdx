...
Supported providers today: **[Daytona](https://www.daytona.io)** and **[E2B](https://e2b.dev)**. Configure one provider per tenant under **Settings → Sandbox providers**.

### Daytona

Pick the Daytona preset from the catalog and paste your API key.
...
Pick the E2B preset and paste your [E2B API key](https://e2b.dev/dashboard?tab=keys). TrueForge builds a template from the release sandbox image in your E2B account (async on first configure), then creates sandboxes from that template.

Advanced settings expose **exec timeout** and **sandbox lifetime**. Auto-archive / auto-delete apply to Daytona only.
...
- Stopped after idle time, then archived and eventually deleted per the provider settings (Daytona), or after the configured sandbox lifetime (E2B).
